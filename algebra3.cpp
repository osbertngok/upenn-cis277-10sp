...
vec3::vec3(const vec4& v) // refind version to avoid divide-by-zero
{ n[VX] = v.n[VX]; n[VY] = v.n[VY]; n[VZ] = v.n[VZ]; }
...
vec3 ratio(const vec3& a, const vec3& b, double Ratio)
{
...
mat4 rotation3D(const vec3& AxisPara, const double angleDeg, const vec3& backup) {
    bool _backup_flag = false;
    if (AxisPara.length() < 0.00001)
    {
      if (abs(angleDeg) < 0.001)
        return identity3D();
      if (abs(angleDeg - 180) < 0.001)
      {
        _backup_flag = true;
      }
    }
    ...
    if (_backup_flag)
    {
      Axis = backup ^ vec3(1,0,0);
      if (Axis.length() < 0.001)
        Axis = backup ^ vec3(0,0,1);
    }
    else
      Axis = AxisPara;
    Axis.normalize();
